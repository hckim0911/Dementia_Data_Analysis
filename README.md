...
**Describe**<br>
* 인자로 받은 DataFrame을 지정한 경로와 이름으로 CSV 파일로 저장합니다.<br>

**Parameter**
| 매개변수 | 타입 | 설명 | 기본값 |
|---|---|---|---|
| `_df` | `pd.DataFrame` | CSV 파일로 저장할 pandas DataFrame입니다. | (필수) |
| `_name` | `str` | 저장할 CSV 파일의 이름입니다. | 'saveData.csv' |
| `_path` | `str` | 파일을 저장할 경로입니다. | 현재 작업 디렉토리 |
